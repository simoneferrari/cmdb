import cmdb_en from '@/modules/cmdb/lang/en.js'
import cs_en from '../views/setting/lang/en.js'
import acl_en from '@/modules/acl/lang/en.js'

export default {
    commonMenu: {
        permission: 'Permission',
        role: 'Roles',
        resource: 'Resources',
        resourceType: 'Resource Types',
        trigger: 'Triggers',
    },
    settings: 'Common Settings',
    screen: 'Big Screen',
    dashboard: 'Dashboard',
    admin: 'Admin',
    user: 'User',
    role: 'Role',
    operation: 'Operation',
    login: 'Login',
    refresh: 'Refresh',
    cancel: 'Cancel',
    confirm: 'Confirm',
    create: 'Create',
    edit: 'Edit',
    deleting: 'Deleting',
    deletingTip: 'Deleting, total of {total}, {successNum} succeeded, {errorNum} failed',
    grant: 'Grant',
    revoke: 'Revoke',
    login_at: 'Login At',
    logout_at: 'Logout At',
    createSuccess: 'Create Success',
    editSuccess: 'edit Success',
    warning: 'Warning',
    export: 'Export',
    placeholderSearch: 'Please Search',
    success: 'Success',
    fail: 'Fail',
    browser: 'Browser',
    status: 'Status',
    type: 'Type',
    description: 'Description',
    new: 'New',
    add: 'Add',
    define: 'Define',
    update: 'Update',
    clear: 'Clear',
    delete: 'Delete',
    copy: 'Copy',
    created_at: 'Created At',
    updated_at: 'Updated At',
    placeholder1: 'Please Input',
    placeholder2: 'Please Select',
    confirmDelete: 'Confirm delete?',
    confirmDelete2: 'Confirm delete [{name}]?',
    query: 'Query',
    search: 'Search',
    hide: 'Hide',
    expand: 'Expand',
    save: 'Save',
    submit: 'Submit',
    upload: 'Import',
    download: 'Export',
    name: 'Name',
    alias: 'Alias',
    desc: 'Description',
    other: 'Other',
    icon: 'Icon',
    addSuccess: 'Added successfully',
    uploadSuccess: 'Import successfully',
    saveSuccess: 'Save successfully',
    copySuccess: 'Copy successfully',
    updateSuccess: 'Updated successfully',
    deleteSuccess: 'Deleted successfully',
    operateSuccess: 'The operation was successful',
    noPermission: 'No Permission',
    noData: 'No Data',
    seconds: 'Seconds',
    createdAt: 'Created At',
    updatedAt: 'Updated At',
    deletedAt: 'Deleted At',
    required: 'required',
    email: 'Email',
    wechat: 'Wechat',
    dingding: 'DingTalk',
    feishu: 'Feishu',
    bot: 'Robot',
    checkAll: 'Select All',
    loading: 'Loading...',
    view: 'View',
    reset: 'Reset',
    yes: 'Yes',
    no: 'No',
    all: 'All',
    selectRows: 'Selected: {rows} items',
    itemsPerPage: '/page',
    '星期一': 'Monday',
    '星期二': 'Tuesday',
    '星期三': 'Wednesday',
    '星期四': 'Thursday',
    '星期五': 'Friday',
    '星期六': 'Saturday',
    '星期日': 'Sunday',
    hour: 'hour',
    'items/page': '{items} items/page',
    max: 'Max',
    min: 'Min',
    visual: 'Visual',
    default: 'default',
    tip: 'Tip',
    cmdbSearch: 'Search',
    requestError: 'An error occurred, please try again later',
    requestServiceError: 'Unknown error on the server, please contact the administrator',
    requestWait: 'The modification has been submitted, please wait for review ({time} seconds)',
    documentCenter: 'Document Center',
    exception: {
        backToHome: 'Back to home page',
        desc1: 'Sorry, you are not authorized to access this page',
        desc2: 'Sorry, the page you are visiting does not exist or is still under development',
        desc3: 'Sorry, server error'
    },
    pagination: {
        total: '{range0}-{range1} of {total} items'
    },
    components: {
        colorTagSelectTip: 'Enter or select tags',
        database: 'Database',
        system: 'System',
        language: 'Language',
        status: 'Status',
        commonComponent: 'Common Component',
        data: 'Data',
        cloud: 'Cloud'
    },
    topMenu: {
        personalCenter: 'My Profile',
        logout: 'Logout',
        confirmLogout: 'Are you sure to log out?'
    },
    cmdbFilterComp: {
        conditionFilter: 'Conditional filtering',
        and: 'and',
        or: 'or',
        is: 'equal',
        '~is': 'not equal',
        contain: 'contain',
        '~contain': 'not contain',
        start_with: 'start_with',
        '~start_with': 'not start_with',
        end_with: 'end_with',
        '~end_with': 'not end_with',
        '~value': 'null',
        value: 'not null',
        in: 'in',
        '~in': 'not in',
        range: 'range',
        '~range': 'out of range',
        compare: 'compare',
        addHere: 'Add Here',
        split: 'split by {separator}'
    },
    customIconSelect: {
        outlined: 'Outlined',
        filled: 'Filled',
        multicolor: 'Multicolor',
        custom: 'Custom',
        preview: 'Preview',
        sizeLimit: 'The image size cannot exceed 2MB!',
        nodata: 'There are currently no custom icons available. Click here to upload'
    },
    regexSelect: {
        limitedFormat: 'Limited Format',
        regExp: 'RegExp',
        errMsg: 'Error Message',
        test: 'Test',
        placeholder: 'Please Select RegExp',
        error: 'Error',
        letter: 'letter',
        number: 'number',
        letterAndNumber: 'letter&number',
        phone: 'phone',
        landline: 'landline',
        zipCode: 'zip code',
        IDCard: 'ID card',
        ip: 'IPv4',
        email: 'email',
        link: 'link',
        monetaryAmount: 'monetary amount',
        custom: 'custom',
    },
    userPanel: {
      myProfile: 'My Profile',
      accountPassword: 'Password',
      notice: 'Notice',
      switchLanguage: 'Switch Language',
      bindAccount: 'Bind Account',
      switchAccount: 'Switch Account',
      logout: 'Logout'
    },
    layout: {
        platformTagline: 'Automation Unified Operations Platform'
    },
    userAuth: {
        register: 'Register',
        emailPlaceholder: 'Email',
        emailRequired: 'Please enter an email address',
        passwordStrength: 'Strength:',
        passwordHint: 'Please enter at least 6 characters. Do not use an easy-to-guess password.',
        passwordPlaceholder: 'At least 6 characters, case sensitive',
        passwordRequired: 'At least 6 characters, case sensitive',
        confirmPasswordPlaceholder: 'Confirm password',
        phonePlaceholder: '11-digit phone number',
        phoneRequired: 'Please enter a valid phone number',
        captchaPlaceholder: 'Captcha',
        captchaRequired: 'Please enter the captcha',
        getCaptcha: 'Get Captcha',
        registerButton: 'Register',
        useExistingAccount: 'Login with existing account',
        strengthLow: 'Low',
        strengthMedium: 'Medium',
        strengthStrong: 'Strong',
        passwordNotStrong: 'Password is not strong enough',
        enterPassword: 'Please enter a password',
        passwordMismatch: 'Passwords do not match',
        captchaSending: 'Sending captcha...',
        captchaTip: 'Tip',
        captchaSuccess: 'Captcha sent successfully. Your captcha is: ',
        error: 'Error',
        requestError: 'An error occurred, please try again later',
        checkEmail: 'Check Email',
        backToHome: 'Back to Home',
        activationEmailSent: 'The activation email has been sent to your inbox. The email is valid for 24 hours. Please log in to your email promptly and click the link to activate your account.',
        accountCreated: 'Your account: {email} has been registered successfully',
        redirectingTo: 'You are about to be redirected to {authType}',
        autoRedirectIn: 'seconds before auto redirect'
    },
    multiTab: {
        noTabsLeft: 'No tabs on the left',
        noTabsRight: 'No tabs on the right',
        closeCurrent: 'Close Current Tab',
        closeRight: 'Close Right',
        closeLeft: 'Close Left',
        closeAll: 'Close All'
    },
    crontab: {
        second: 'Second',
        minute: 'Minute',
        hour: 'Hour',
        day: 'Day',
        month: 'Month',
        week: 'Week',
        year: 'Year',
        timeExpression: 'Time Expression',
        fullExpression: 'Full Crontab Expression',
        ok: 'OK',
        reset: 'Reset',
        cancel: 'Cancel'
    },
    noticeCenter: {
        title: 'Notification Center',
        searchPlaceholder: 'Enter search content',
        markAsRead: 'Mark as Read',
        markAsUnread: 'Mark as Unread',
        selected: 'Selected: {count} items',
        titleContent: 'Title Content',
        submitTime: 'Submit Time',
        type: 'Type',
        allMessages: 'All Messages',
        unreadMessages: 'Unread Messages',
        readMessages: 'Read Messages',
        all: 'All'
    },
    monitorNode: {
        ipAddress: 'IP Address',
        enterIpAddress: 'Please enter IP address',
        ipFormatError: 'IP address format error',
        enterCommunity: 'Please enter community',
        version: 'Version',
        enterVersion: 'Please enter version',
        selectVersion: 'Please select version',
        addNode: 'Add Node',
        cannotDelete: 'Cannot delete anymore!'
    },
    permissions: {
        add: 'Add',
        delete: 'Delete',
        edit: 'Edit',
        query: 'Query',
        get: 'Details',
        enable: 'Enable',
        disable: 'Disable',
        import: 'Import',
        export: 'Export',
        companyInfo: 'Company Info',
        companyStructure: 'Company Structure',
        userGroup: 'User Group',
        noticeSettings: 'Notification Settings',
        operationPermission: 'Operation Permission',
        modelConfig: 'Model Configuration'
    },
    employeeTransfer: {
        empty: 'Empty',
        fullTime: 'Full-time',
        intern: 'Intern'
    },
    welcomeMessages: {
        msg1: 'Take a break',
        msg2: 'What are you going to eat?',
        msg3: 'How about playing a game?',
        msg4: 'I guess you might be tired',
        msg5: 'Do you have a small goal?'
    },
    graphToolbar: {
        fullscreen: 'Fullscreen',
        exitFullscreen: 'Exit Fullscreen',
        zoomIn: 'Zoom In',
        zoomOut: 'Zoom Out',
        layout: 'Layout',
        lineStyle: 'Line Style',
        straight: 'Straight',
        simple: 'Simple',
        vivid: 'Vivid',
        fishtail: 'Fishtail',
        polyline: 'Polyline',
        connectionPoint: 'Connection Point',
        edge: 'Edge',
        fourPoints: 'Four Points',
        topBottom: 'Top-Bottom',
        leftRight: 'Left-Right',
        auto: 'Auto',
        refresh: 'Refresh',
        download: 'Download',
        startAutoLayout: 'Click to start automatic layout adjustment',
        stopAutoLayout: 'Click to stop automatic layout',
        layoutNotSupported: 'Current layout does not support auto layout!'
    },
    iconLabels: {
        // Status
        question: 'Question',
        pause: 'Pause',
        add: 'Add',
        subtract: 'Subtract',
        cancel: 'Cancel',
        confirm: 'Confirm',
        warning: 'Warning',
        forbidden: 'Forbidden',
        description: 'Description',
        processing: 'Processing',
        online: 'Online',
        offline: 'Offline',
        // Common components
        connected: 'Connected',
        disconnected: 'Disconnected',
        favorites: 'Favorites',
        alarm: 'Alarm',
        qrCode: 'QR Code',
        branch: 'Branch',
        cloudData: 'Cloud Data',
        cloudUpload: 'Cloud Upload',
        cloudDownload: 'Cloud Download',
        message: 'Message',
        location: 'Location',
        view: 'View',
        noViewing: 'No Viewing',
        verified: 'Verified',
        unverified: 'Unverified',
        tag: 'Tag',
        user: 'User',
        addUser: 'Add User',
        deleteUser: 'Delete User',
        switchUser: 'Switch User',
        maintenance: 'Maintenance',
        physicalMachine: 'Physical Machine',
        virtualMachine: 'Virtual Machine',
        router: 'Router',
        switch: 'Switch',
        firewall: 'Firewall',
        loadBalancer: 'Load Balancer',
        nic: 'NIC',
        memory: 'Memory',
        hardDisk: 'Hard Disk',
        department: 'Department',
        product: 'Product',
        printer: 'Printer',
        plugin: 'Plugin',
        application: 'Application',
        alibabaCloud: 'Alibaba Cloud',
        tencentCloud: 'Tencent Cloud',
        huaweiCloud: 'Huawei Cloud',
        // Charts
        pieChart: 'Pie Chart',
        ringChart: 'Ring Chart',
        barChart: 'Bar Chart',
        horizontalBar: 'Horizontal Bar',
        areaChart: 'Area Chart',
        waterfallChart: 'Waterfall Chart',
        boxPlot: 'Box Plot',
        lineChart: 'Line Chart',
        scatterPlot: 'Scatter Plot',
        heatMap: 'Heat Map',
        rising: 'Rising',
        falling: 'Falling',
        stockChart: 'Stock Chart',
        fundChart: 'Fund Chart',
        sliderChart: 'Slider Chart',
        radarChart: 'Radar Chart',
        integer: 'Integer',
        float: 'Float',
        text: 'Text',
        // Multicolor specific
        dameng: 'Dameng',
        ctyun: 'CTYun',
        ecloud: 'ECloud',
        jdCloud: 'JD Cloud',
        byteCloud: 'ByteCloud',
        tongtech: 'Tongtech',
        diskArray: 'Disk Array',
        fiberSwitch: 'Fiber Switch',
        bandwidthLine: 'Bandwidth Line',
        rack: 'Rack',
        computerRoom: 'Computer Room',
        ipAddress: 'IP Address',
        ipPool: 'IP Pool',
        storageVolume: 'Storage Volume',
        virtualization: 'Virtualization',
        business: 'Business',
        database: 'Database',
        middleware: 'Middleware',
        messageQueue: 'Message Queue',
        storageDevice: 'Storage Device',
        networkDevice: 'Network Device',
        computer: 'Computer',
        hardware: 'Hardware',
        dataCenter: 'Data Center',
        operatingSystem: 'Operating System',
        publicCloud: 'Public Cloud',
        folder: 'Folder',
        resourcePool: 'Resource Pool',
        network: 'Network',
        distributedSwitch: 'Distributed Switch',
        standardSwitch: 'Standard Switch',
        hostCluster: 'Host Cluster',
        dataStorageCluster: 'Data Storage Cluster',
        dataStorage: 'Data Storage',
        objectStorage: 'Object Storage'
    },
    cmdb: cmdb_en,
    cs: cs_en,
    acl: acl_en,
}
