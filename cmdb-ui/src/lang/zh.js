import cmdb_zh from '@/modules/cmdb/lang/zh.js'
import cs_zh from '../views/setting/lang/zh.js'
import acl_zh from '@/modules/acl/lang/zh.js'

export default {
    commonMenu: {
        permission: '权限管理',
        role: '角色管理',
        resource: '资源管理',
        resourceType: '资源类型',
        trigger: '触发器',
    },
    settings: '通用设置',
    screen: '大屏',
    dashboard: '仪表盘',
    admin: '管理员',
    user: '用户',
    role: '角色',
    operation: '操作',
    login: '登录',
    refresh: '刷新',
    cancel: '取消',
    confirm: '确定',
    create: '创建',
    edit: '编辑',
    deleting: '正在删除',
    deletingTip: '正在删除，共{total}个，成功{successNum}个，失败{errorNum}个',
    grant: '授权',
    revoke: '回收',
    login_at: '登录时间',
    logout_at: '登出时间',
    createSuccess: '创建成功',
    editSuccess: '修改成功',
    warning: '警告',
    export: '导出',
    placeholderSearch: '请查找',
    success: '成功',
    fail: '失败',
    browser: '浏览器',
    status: '状态',
    type: '类型',
    description: '描述',
    new: '新增',
    add: '添加',
    define: '定义',
    update: '修改',
    clear: '清空',
    delete: '删除',
    copy: '复制',
    created_at: '创建日期',
    updated_at: '更新日期',
    placeholder1: '请输入',
    placeholder2: '请选择',
    confirmDelete: '确认删除？',
    confirmDelete2: '确认删除【{name}】?',
    query: '查询',
    search: '搜索',
    hide: '隐藏',
    expand: '展开',
    save: '保存',
    submit: '提交',
    upload: '导入',
    download: '导出',
    name: '名称',
    alias: '别名',
    desc: '描述',
    other: '其他',
    icon: '图标',
    addSuccess: '新增成功',
    uploadSuccess: '导入成功',
    saveSuccess: '保存成功',
    copySuccess: '复制成功',
    updateSuccess: '更新成功',
    deleteSuccess: '删除成功',
    operateSuccess: '操作成功',
    noPermission: '权限不足',
    noData: '暂无数据',
    seconds: '秒',
    createdAt: '创建时间',
    updatedAt: '更新时间',
    deletedAt: '删除时间',
    required: '必须',
    email: '邮件',
    wechat: '企业微信',
    dingding: '钉钉',
    feishu: '飞书',
    bot: '机器人',
    checkAll: '全选',
    loading: '加载中...',
    view: '查看',
    reset: '重置',
    yes: '是',
    no: '否',
    all: '全部',
    selectRows: '选取：{rows} 项',
    itemsPerPage: '/页',
    '星期一': '星期一',
    '星期二': '星期二',
    '星期三': '星期三',
    '星期四': '星期四',
    '星期五': '星期五',
    '星期六': '星期六',
    '星期日': '星期日',
    hour: '小时',
    'items/page': '{items} 条/页',
    max: '最大值',
    min: '最小值',
    visual: '虚拟',
    default: '默认',
    tip: '提示',
    cmdbSearch: '搜索一下',
    requestError: '出现错误，请稍后再试',
    requestServiceError: '服务端未知错误, 请联系管理员！',
    requestWait: '修改已提交，请等待审核（{time}s）',
    documentCenter: '文档中心',
    exception: {
        backToHome: '返回首页',
        desc1: '抱歉，你无权访问该页面',
        desc2: '抱歉，你访问的页面不存在或仍在开发中',
        desc3: '抱歉，服务器出错了'
    },
    pagination: {
        total: '当前展示 {range0}-{range1} 条数据， 共 {total} 条'
    },
    components: {
        colorTagSelectTip: '选择或输入（回车确定）标签',
        database: '数据库',
        system: '操作系统',
        language: '语言',
        status: '状态',
        commonComponent: '常用组件',
        data: '数据',
        cloud: '云'
    },
    topMenu: {
        personalCenter: '个人中心',
        logout: '退出登录',
        confirmLogout: '确认退出登录吗？'
    },
    cmdbFilterComp: {
        conditionFilter: '条件过滤',
        and: '与',
        or: '或',
        is: '等于',
        '~is': '不等于',
        contain: '包含',
        '~contain': '不包含',
        start_with: '以...开始',
        '~start_with': '不以...开始',
        end_with: '以...结束',
        '~end_with': '不以...结束',
        '~value': '为空',
        value: '不为空',
        in: 'in查询',
        '~in': '非in查询',
        range: '范围',
        '~range': '范围外',
        compare: '比较',
        addHere: '在此处添加',
        split: '以 {separator} 分隔'
    },
    customIconSelect: {
        outlined: '线框',
        filled: '实底',
        multicolor: '多色',
        custom: '自定义',
        preview: '预览',
        sizeLimit: '图片大小不可超过2MB！',
        nodata: '暂无自定义图标，点击此处上传'
    },
    regexSelect: {
        limitedFormat: '限定格式',
        regExp: '正则表达式',
        errMsg: '错误时提示',
        test: '测试',
        placeholder: '请选择正则表达式',
        error: '错误',
        letter: '字母',
        number: '数字',
        letterAndNumber: '字母和数字',
        phone: '手机号码',
        landline: '座机',
        zipCode: '邮政编码',
        IDCard: '身份证号',
        ip: 'IPv4地址',
        email: '邮箱',
        link: '链接',
        monetaryAmount: '货币金额',
        custom: '自定义',
    },
    userPanel: {
      myProfile: '个人中心',
      accountPassword: '账号密码',
      notice: '通知中心',
      switchLanguage: '切换语言',
      bindAccount: '绑定账号',
      switchAccount: '切换账号',
      logout: '退出账号'
    },
    layout: {
        platformTagline: '自动化统一运维平台'
    },
    userAuth: {
        register: '注册',
        emailPlaceholder: '邮箱',
        emailRequired: '请输入邮箱地址',
        passwordStrength: '强度：',
        passwordHint: '请至少输入 6 个字符。请不要使用容易被猜到的密码。',
        passwordPlaceholder: '至少6位密码，区分大小写',
        passwordRequired: '至少6位密码，区分大小写',
        confirmPasswordPlaceholder: '确认密码',
        phonePlaceholder: '11 位手机号',
        phoneRequired: '请输入正确的手机号',
        captchaPlaceholder: '验证码',
        captchaRequired: '请输入验证码',
        getCaptcha: '获取验证码',
        registerButton: '注册',
        useExistingAccount: '使用已有账户登录',
        strengthLow: '低',
        strengthMedium: '中',
        strengthStrong: '强',
        passwordNotStrong: '密码强度不够',
        enterPassword: '请输入密码',
        passwordMismatch: '两次密码不一致',
        captchaSending: '验证码发送中..',
        captchaTip: '提示',
        captchaSuccess: '验证码获取成功，您的验证码为：',
        error: '错误',
        requestError: '请求出现错误，请稍后再试',
        checkEmail: '查看邮箱',
        backToHome: '返回首页',
        activationEmailSent: '激活邮件已发送到你的邮箱中，邮件有效期为24小时。请及时登录邮箱，点击邮件中的链接激活帐户。',
        accountCreated: '你的账户：{email} 注册成功',
        redirectingTo: '您即将跳转至{authType}',
        autoRedirectIn: '秒后自动跳转'
    },
    multiTab: {
        noTabsLeft: '左侧没有标签',
        noTabsRight: '右侧没有标签',
        closeCurrent: '关闭当前标签',
        closeRight: '关闭右侧',
        closeLeft: '关闭左侧',
        closeAll: '关闭全部'
    },
    crontab: {
        second: '秒',
        minute: '分钟',
        hour: '小时',
        day: '日',
        month: '月',
        week: '周',
        year: '年',
        timeExpression: '时间表达式',
        fullExpression: 'crontab完整表达式',
        ok: '确定',
        reset: '重置',
        cancel: '取消'
    },
    noticeCenter: {
        title: '消息中心',
        searchPlaceholder: '请输入你需要搜索的内容',
        markAsRead: '标为已读',
        markAsUnread: '标为未读',
        selected: '选取: {count} 项',
        titleContent: '标题内容',
        submitTime: '提交时间',
        type: '类型',
        allMessages: '全部消息',
        unreadMessages: '未读消息',
        readMessages: '已读消息',
        all: '全部'
    },
    monitorNode: {
        ipAddress: 'ip地址',
        enterIpAddress: '请输入ip地址',
        ipFormatError: 'ip地址格式错误',
        enterCommunity: '请输入community',
        version: '版本',
        enterVersion: '请输入版本',
        selectVersion: '请选择版本',
        addNode: '添加节点',
        cannotDelete: '不可再删除！'
    },
    permissions: {
        add: '新增',
        delete: '删除',
        edit: '修改',
        query: '查询',
        get: '详情',
        enable: '启用',
        disable: '禁用',
        import: '导入',
        export: '导出',
        companyInfo: '公司信息',
        companyStructure: '公司架构',
        userGroup: '用户分组',
        noticeSettings: '通知设置',
        operationPermission: '操作权限',
        modelConfig: '模型配置'
    },
    employeeTransfer: {
        empty: '空',
        fullTime: '全职',
        intern: '实习生'
    },
    welcomeMessages: {
        msg1: '休息一会儿吧',
        msg2: '准备吃什么呢?',
        msg3: '要不要打一把 DOTA?',
        msg4: '我猜你可能累了',
        msg5: '你有一个小目标吗?'
    },
    graphToolbar: {
        fullscreen: '全屏',
        exitFullscreen: '退出',
        zoomIn: '放大',
        zoomOut: '缩小',
        layout: '布局',
        lineStyle: '线条',
        straight: '直线',
        simple: '简洁',
        vivid: '生动',
        fishtail: '鱼尾',
        polyline: '折线',
        connectionPoint: '连接点',
        edge: '边缘',
        fourPoints: '四点',
        topBottom: '上下',
        leftRight: '左右',
        auto: '自动',
        refresh: '刷新',
        download: '下载',
        startAutoLayout: '点击开始自动调整布局',
        stopAutoLayout: '点击停止自动布局',
        layoutNotSupported: '当前布局不支持自动布局！'
    },
    iconLabels: {
        // Status
        question: '疑问',
        pause: '暂停',
        add: '添加',
        subtract: '减去',
        cancel: '取消',
        confirm: '确认',
        warning: '警告',
        forbidden: '禁止',
        description: '说明',
        processing: '处理中',
        online: '在线',
        offline: '下线',
        // Common components
        connected: '已连接',
        disconnected: '未连接',
        favorites: '收藏',
        alarm: '报警',
        qrCode: '二维码',
        branch: '分支',
        cloudData: '云数据',
        cloudUpload: '云上传',
        cloudDownload: '云下载',
        message: '消息',
        location: '定位',
        view: '观看',
        noViewing: '禁止观看',
        verified: '已认证',
        unverified: '未认证',
        tag: '标签',
        user: '用户',
        addUser: '添加用户',
        deleteUser: '删除用户',
        switchUser: '切换用户',
        maintenance: '维修',
        physicalMachine: '物理机',
        virtualMachine: '虚拟机',
        router: '路由器',
        switch: '交换机',
        firewall: '防火墙',
        loadBalancer: '负载均衡',
        nic: '网卡',
        memory: '内存',
        hardDisk: '硬盘',
        department: '部门',
        product: '产品',
        printer: '打印机',
        plugin: '插件',
        application: '应用',
        alibabaCloud: '阿里云',
        tencentCloud: '腾讯云',
        huaweiCloud: '华为云',
        // Charts
        pieChart: '饼状图',
        ringChart: '环形图',
        barChart: '柱状图',
        horizontalBar: '条形图',
        areaChart: '面积图',
        waterfallChart: '瀑布图',
        boxPlot: '箱型图',
        lineChart: '折线图',
        scatterPlot: '点状图',
        heatMap: '热度图',
        rising: '上升',
        falling: '下降',
        stockChart: '股票图',
        fundChart: '基金图',
        sliderChart: '滑块图',
        radarChart: '雷达图',
        integer: '整数',
        float: '浮点数',
        text: '文本',
        // Multicolor specific
        dameng: '达梦',
        ctyun: '天翼云',
        ecloud: '移动云',
        jdCloud: '京东云',
        byteCloud: '字节云',
        tongtech: '东方通',
        diskArray: '磁盘阵列',
        fiberSwitch: '光纤交换机',
        bandwidthLine: '带宽线路',
        rack: '机柜',
        computerRoom: '机房',
        ipAddress: 'ip地址',
        ipPool: 'ip池',
        storageVolume: '存储卷',
        virtualization: '虚拟化',
        business: '业务',
        database: '数据库',
        middleware: '中间件',
        messageQueue: '消息队列',
        storageDevice: '存储设备',
        networkDevice: '网络设备',
        computer: '计算机',
        hardware: '硬件设备',
        dataCenter: '数据中心',
        operatingSystem: '操作系统',
        publicCloud: '公有云',
        folder: '文件夹',
        resourcePool: '资源池',
        network: '网络',
        distributedSwitch: '分布式交换机',
        standardSwitch: '标准式交换机',
        hostCluster: '主机集群',
        dataStorageCluster: '数据存储集群',
        dataStorage: '数据存储',
        objectStorage: '对象存储'
    },
    cmdb: cmdb_zh,
    cs: cs_zh,
    acl: acl_zh,
}
